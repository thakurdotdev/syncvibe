...
const ShareDrawer = ({ postid, shareLink, isOpen, onClose }) => {
  ...
  const postUrl = postid ? `${window.location.origin}/post/${postid}` : shareLink;

  const handleShare = async (platform) => {
    const shareUrls = {
      facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(postUrl)}`,
      twitter: `https://twitter.com/intent/tweet?url=${encodeURIComponent(postUrl)}`,
      whatsapp: `https://api.whatsapp.com/send?text=${encodeURIComponent(postUrl)}`,
    };
            ...
            <div className='mb-8 bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-4 backdrop-blur-sm'>
              <div className='flex items-center justify-between mb-3'>
                ...
                <Button
                  variant='ghost'
                  size='sm'
                  className={cn('gap-2 transition-all', copied && 'text-green-500')}
