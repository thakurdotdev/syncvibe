const getClientIp = (req) => {
  const cfIp =
    ...
    (typeof req.get === 'function' && req.get('cf-connecting-ip'));
  ...
  const forwarded =
    ...
    (typeof req.get === 'function' && req.get('x-forwarded-for'));
  ...
  const realIp =
    req.headers?.['x-real-ip'] || (typeof req.get === 'function' && req.get('x-real-ip'));
...
const getClientLocation = (req) => {
  const cityHeader =
    req.headers?.['cf-ipcity'] || (typeof req.get === 'function' && req.get('cf-ipcity'));
  const regionHeader =
    req.headers?.['cf-region'] ||
    req.headers?.['cf-region-code'] ||
    (typeof req.get === 'function' && req.get('cf-region'));
  const countryHeader =
    req.headers?.['cf-ipcountry'] || (typeof req.get === 'function' && req.get('cf-ipcountry'));
...
const getBrowser = (req, userAgent) => {
  const secChUa =
    req.headers?.['sec-ch-ua'] || (typeof req.get === 'function' && req.get('sec-ch-ua')) || '';

  if (
    /Brave/i.test(secChUa) ||
    req.headers?.['x-brave-custom-header'] ||
    (typeof req.get === 'function' && req.get('sec-gpc')) ||
