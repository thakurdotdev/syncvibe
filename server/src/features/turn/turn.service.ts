...
const filterBlockedPorts = (urls: string[]): string[] => {
  return urls.filter((url) => !url.includes(':53'));
};

export const generateTurnCredentials = async (): Promise<TurnCredentials> => {
  const now = Date.now();
  ...
  const iceServers: IceServer[] = data.iceServers.map((server) => ({
    ...server,
    urls: filterBlockedPorts(Array.isArray(server.urls) ? server.urls : [server.urls]),
  }));
