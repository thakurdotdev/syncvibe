  ...
  socket.on('call-user', async (data) => {
    ...
    try {
      ...
      const recipientSocket =
        userSockets.get(recipientKey) || userSockets.get(Number(to)) || userSockets.get(to);
      if (!recipientSocket) {
      ...
      if (!onlineUsers.has(recipientKey) && !onlineUsers.has(Number(to))) {
        sendPushNotification(to, { name: data.name, from: socket.userId }, 'call');
      }
    } catch (error) {
