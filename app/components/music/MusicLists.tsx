...
export const MusicQueue = memo(() => {
  const { colors } = useTheme()
  const { reorderPlaylist, clearQueue, addToQueue, removeFromQueue, removeFromQueueBelow } =
    usePlayerControls()
  ...
  const { data: recommendations = [], isFetched: recsFetched } = useSongRecommendationsQuery(
    recTargetId,
    {
      enabled: !!recTargetId,
    },
  ...
  if (!playlist.length) {
    return (
      <Animated.View entering={FadeIn.duration(400)} style={queueStyles.emptyContainer}>
