...
const SkeletonCard = memo(({ colors }: { colors: any }) => (
  <View style={[skeletonStyles.card, { backgroundColor: colors.card }]}>
    ...
    <View style={skeletonStyles.textArea}>
      <View style={[skeletonStyles.titleLine, { backgroundColor: colors.muted }]} />
      <View style={[skeletonStyles.subtitleLine, { backgroundColor: colors.muted }]} />
    </View>
  </View>
))
...
const RecommendationItem = memo(
  ({ song, onAdd, colors }: { song: Song; onAdd: (song: Song) => void; colors: any }) => {
    const artistName = song.subtitle || song.artist_map?.artists?.[0]?.name || "Unknown"
    const imageUrl = song.image?.[1]?.link

    return (
      <Animated.View entering={FadeIn.duration(300)}>
        <View style={[recItemStyles.container, { backgroundColor: colors.card }]}>
          <Image source={{ uri: imageUrl }} style={recItemStyles.image} resizeMode="cover" />
          <View style={recItemStyles.info}>
            <Text style={[recItemStyles.title, { color: colors.text }]} numberOfLines={1}>
              {song.name}
            </Text>
            <Text
              style={[recItemStyles.artist, { color: colors.mutedForeground }]}
              numberOfLines={1}
            >
              {artistName}
            </Text>
  ...
  return (
    ...
    >
      <View style={containerStyles.header}>
        <Text style={[containerStyles.title, { color: colors.text }]}>Recommended</Text>
      ...
      {isLoading ? (
        ...
        <View style={containerStyles.emptyState}>
          ...
          <Text style={[containerStyles.emptyText, { color: colors.mutedForeground }]}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <RecommendationItem song={item} onAdd={handleAdd} colors={colors} />
          )}
          scrollEnabled={false}
